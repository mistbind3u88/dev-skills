...
### 6. プランニングまたはタスクドキュメント作成

次セッションへのハンドオーバーのため、**プランファイルまたはタスクドキュメントのいずれかを必ず作成する**。

#### 6a. プランモード（対応内容が明確な場合）
...
#### 6b. タスクドキュメント（対応内容が曖昧な場合、Issue がない場合）

プランモードに入らない場合でも、元ディレクトリの `.claude/docs` 配下にタスクドキュメントを作成する。

タスクドキュメントには以下を含める:

- ブランチ名と worktree パス
- 開発の背景・目的（Issue があればリンク）
- ここまでの調査で判明した情報（依存関係の差分、影響範囲など）
- 次セッションで着手すべき作業の概要
